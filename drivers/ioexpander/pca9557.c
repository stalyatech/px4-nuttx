...
static FAR void *pca9557_attach(FAR struct ioexpander_dev_s *dev,
             ioe_pinset_t pinset, ioe_callback_t callback, FAR void *arg);
static int pca9557_detach(FAR struct ioexpander_dev_s *dev,
             FAR void *handle);
#endif
...
static int pca9557_lock(FAR struct pca9557_dev_s *pca)
{
  return nxsem_wait_uninterruptible(&pca->exclsem);
}

#define pca9557_unlock(p) nxsem_post(&(p)->exclsem)
  ...
  if (opt == IOEXPANDER_OPTION_INVERT)
    {
      ...
      ret = pca9557_setbit(pca, PCA9557_REG_POLINV, pin, ival);
      pca9557_unlock(pca);
    }
                                ...
                                int count)
{
  ...
  ret = pca9557_writeread(pca, &addr, 1, buf, 2);
  if (ret < 0)
    {
      return ret;
    }
                                 ...
                                 int count)
{
  ...
  ret = pca9557_writeread(pca, &addr, 1, &buf[1], 2);
  if (ret < 0)
    {
      pca9557_unlock(pca);
      return ret;
...
#ifdef CONFIG_PCA9557_INT_ENABLE
                                ...
                                FAR void *arg)
{
  ...
  for (i = 0; i < CONFIG_PCA9557_INT_NCALLBACKS; i++)
    {
 ...
 * Input Parameters:
 *   dev      - Device-specific state data
 *   handle   - The non-NULL opaque value return by pca9557_attch()
 *
 * Returned Value:
 *   0 on success, else a negative error code
 *
...
static int pca9557_detach(FAR struct ioexpander_dev_s *dev, FAR void *handle)
{
  FAR struct pca9557_dev_s *pca = (FAR struct pca9557_dev_s *)dev;
  FAR struct pca9557_callback_s *cb =
    (FAR struct pca9557_callback_s *)handle;

  DEBUGASSERT(pca != NULL && cb != NULL);
  DEBUGASSERT((uintptr_t)cb >= (uintptr_t)&pca->cb[0] &&
              (uintptr_t)cb <=
              (uintptr_t)&pca->cb[CONFIG_PCA9557_INT_NCALLBACKS - 1]);
  UNUSED(pca);
...
static void pca9557_irqworker(void *arg)
{
  FAR struct pca9557_dev_s *pca = (FAR struct pca9557_dev_s *)arg;
  uint8_t addr = PCA9557_REG_INPUT;
  uint8_t buf[2];
  ioe_pinset_t pinset;
  int ret;
  int i;
...
static int pca9557_interrupt(int irq, FAR void *context, FAR void *arg)
{
  FAR struct pca9557_dev_s *pca = (FAR struct pca9557_dev_s *)arg;
   ...
   * Interrupts are re-enabled in pca9557_irqworker() when the work is
   * completed.
   */
  ...
  if (work_available(&pca->work))
    {
      ...
      work_queue(HPWORK, &pca->work, pca9557_irqworker,
                 (FAR void *)pca, 0);
    }
...
#ifdef CONFIG_PCA9557_INT_ENABLE
  pcadev->config->attach(pcadev->config, pca9557_interrupt, pcadev);
